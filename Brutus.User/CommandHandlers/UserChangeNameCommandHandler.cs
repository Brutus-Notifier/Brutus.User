...
namespace Brutus.User.CommandHandlers
{
    public class UserChangeNameCommandHandler: ICommandHandler<Commands.V1.UserChangeName>
    {
        ...
        {
            await this.HandleUpdate(context, _aggregateRepository, context.Message.UserId,
                x => x.ChangeName(context.Message.FirstName, context.Message.LastName));

            await context.RespondAsync(new Events.V1.SuccessResponse());
        }
    }
}